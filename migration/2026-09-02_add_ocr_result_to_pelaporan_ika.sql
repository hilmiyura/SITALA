-- =============================================================================
-- Menambahkan kolom ocr_result pada tabel pelaporan_ika
--
-- Konteks
--   Alur OCR (/ocr/ikaExtract) saat ini tidak meninggalkan jejak apa pun:
--   dokumen sumber dibaca dari direktori temp PHP lalu dibuang tanpa disimpan,
--   dan hasil ekstraksinya hanya dipakai untuk mengisi form di browser.
--   Akibatnya, ketika suatu nilai dipertanyakan, tidak ada cara menelusuri
--   kembali apa yang sebenarnya dibaca model.
--
--   Kolom ini menyimpan payload JSON hasil ekstraksi (respons ikaExtract,
--   setelah pencocokan ke uid master) supaya tiap baris pelaporan bisa
--   ditelusuri ulang. Menyamakan pelaporan_ika dengan pelaporan_iku yang sudah
--   lebih dulu diberi kolom serupa pada migrasi 2026-09-01.
--
-- Cara menjalankan (manual -- tidak ada migration runner di repo ini):
--   mysql -h <host> -u <user> -p <database> < 2026-09-02_add_ocr_result_to_pelaporan_ika.sql
--
-- Urutan
--   Jalankan berkas ini LEBIH DULU, baru
--   2026-09-02_add_ocr_result_to_v_pelaporan_ika.sql. View tidak bisa dibuat
--   ulang selama kolomnya belum ada di tabel dasar.
--
-- Catatan tipe kolom
--   TEXT menampung maksimal 65.535 byte. Payload IKA lebih besar daripada IKU
--   karena satu dokumen bisa memuat sampai 55 parameter per lokasi, tapi untuk
--   pemakaian normal masih beberapa kilobyte saja.
--   Perlu diketahui: sql_mode server ini TIDAK memuat STRICT_TRANS_TABLES,
--   sehingga nilai yang melebihi kapasitas akan dipotong DIAM-DIAM tanpa error.
--   Karena itu controller memadatkan JSON-nya sebelum menyimpan (json_encode
--   ulang tanpa indentasi). Bila nanti masih membesar, ganti ke MEDIUMTEXT (16 MB).
-- =============================================================================

ALTER TABLE `pelaporan_ika`
  ADD COLUMN `ocr_result` TEXT NULL DEFAULT NULL
  COMMENT 'Payload JSON hasil OCR dari ocrController::ikaExtract'
  AFTER `shu`;


-- -----------------------------------------------------------------------------
-- Verifikasi
-- -----------------------------------------------------------------------------
-- SELECT column_name, column_type, is_nullable, column_comment
--   FROM information_schema.columns
--  WHERE table_schema = DATABASE()
--    AND table_name   = 'pelaporan_ika'
--    AND column_name  = 'ocr_result';


-- -----------------------------------------------------------------------------
-- Rollback
-- -----------------------------------------------------------------------------
-- ALTER TABLE `pelaporan_ika` DROP COLUMN `ocr_result`;
