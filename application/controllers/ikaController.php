...
class ikaController extends Front
{
    ...
    public function index()
    {
        ...
        if (isset($post['submit'])) {
            ...
            if($statusDuplikasi == 0){
              ...
              //views/be/parts/contents/ika/script.html).
                ...
                foreach ($data as $key => $vals) {
                    if ($vals[1] != "-") {
                        ...
                        if ($this -> cekLocation($where, $vals, $post['form']) && $date <= date("Y-m-d")) {
                          ...
                          $postLaporan['form']['uid_lab'] = $this->checkLab(preg_replace('~[\\\\/:*?"<>|]~', "", $vals[61]));
    ...
    {// decode kolom ocr_result (TEXT berisi JSON) jadi array siap pakai di template
        ...
        //Bentuk "lab" berubah sejak OCR IKA mendukung multi-lab: dulu SATU objek
        //{uid, text}, sekarang ARRAY objek karena satu lokasi bisa dilayani beberapa lab
        //yang digabung model lewat laboratorium_text "Lab A; Lab B" (lihat matchFieldsIka()
    ...
    {// decode kolom catatan_ocr (TEXT berisi JSON) jadi array siap pakai di template
        ...
        //lihat buildCatatanOcr() di views/be/parts/contents/ika/script.html). Menyatukan
    ...
    public function dataExcelStatusMutu($w=null, $offset=null, $tahunShow=null)
    {
        $offset = $this->params('offset');
        $this -> tables -> set($this -> viewName, $this -> primaryKey);
        $properties = $this -> _getProperties($this -> viewName);
    ...
    {//function for counting data pelaporan
        ...
        if($dataIndeks['total']) {
          ...
          $parameterSql2025 = $this->ika->parameterIndeks2025();
          $sql = "SELECT a.uid_pelaporan_ika, a.kategori, a.uid_kabkota , a.kode_lokasi, {$parameterSql2025} FROM v_pelaporan_ika a WHERE " . $w;
          $cnIndeks2025 = $this -> tables -> query($sql);

          $cnIndeks2025 = $this->ika->cnIndeks2025($cnIndeks2025);
          $cnIndeks2025 = $cnIndeks2025["cnIndeks"];
          $tmpMemenuhi2025 = $cnIndeks2025["tmpMemenuhi"];
          ...
          $cnIndeks2024 = $this->ika->cnIndeks2024($cnIndeks);
          $cnIndeks = $cnIndeks2024["cnIndeks"];
          $tmpMemenuhi = $cnIndeks2024["tmpMemenuhi"];
          ...
          $postIdx['form']['json_data_ina'] = json_encode($cnIndeks2025['data']);
          $postIdx['form']['nilai_indeks_ina'] = $cnIndeks2025['ika_ina'];
          $postIdx['form']['nilai_indeks_ina_mutu'] = $cnIndeks2025['ika_ina_status'];

          $postIdx['submit'] = true;
          $this -> tables -> set("indeks_ika", "uid_indeks_ika");
          // $this->debug->show($cnIndeks2025);
          if ($this -> tables -> post($postIdx)) {
    ...
    public function _countIndekStatusMutu($idIndeks)
    {
        ...
        if ($cekData['total']) {
            ...
            $parameterSql2025 = $this->ika->parameterIndeks2025();
            $sql = "SELECT a.uid_pelaporan_ika, a.kategori, a.uid_kabkota , {$parameterSql2025} FROM v_pelaporan_ika a WHERE " . $whereIndeksParams;
            $cnIndeks2025 = $this -> tables -> query($sql);
            $update['status_mutu_ina'] = $cnIndeks2025['ika_ina_status'];
            $update['status_mutu_ina_nilai'] = $cnIndeks2025['ika_ina'];
            ...
            $update['status_mutu_ina'] = null;
            $update['status_mutu_nilai_ina'] = null;
        }
        return $update;
    }
    ...
    public function titikLokasi()
    {
        if ($this->params(uid)) {
            ...
            if($rf["data"][0]["tahun"] < 2025){
              $rf['data'][0]['json'] = json_decode($rf['data'][0]['json_data'], true);
            }else{
              $rf['data'][0]['json'] = json_decode($rf['data'][0]['json_data_ina'], true);
            }
            $html = '';
            $i = 1;
